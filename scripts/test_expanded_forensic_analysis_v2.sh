...
echo "=== TEST_EXPANDED_FORENSIC_ANALYSIS_V2 ==="

PYTHONPATH=src python -m py_compile \
  src/scripts/research/build_expanded_forensic_analysis_v2.py

OUT="$(PYTHONPATH=src python src/scripts/research/build_expanded_forensic_analysis_v2.py)"

echo "$OUT"

echo "$OUT" | grep -q "mode=research_only"
echo "$OUT" | grep -q "expanded_forensic_rows="
echo "$OUT" | grep -q "matrix_scope=symbol_timeframe_strategy_regime"
...
echo "$OUT" | grep -q "VERDICT=EXPANDED_FORENSIC_ANALYSIS_V2_READY"
...
with psycopg2.connect(db) as conn:
    with conn.cursor() as cur:
        cur.execute("SELECT count(*) FROM analytics_global_edge_expanded_forensic_v2;")
...
print("db_expanded_forensic_ready=READY")
print("db_classification_ready=READY")
print("db_runtime_block_ready=READY")
PY

echo "VERDICT=TEST_EXPANDED_FORENSIC_ANALYSIS_V2_OK"
