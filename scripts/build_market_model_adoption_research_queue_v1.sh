...
echo "=== BUILD_MARKET_MODEL_ADOPTION_RESEARCH_QUEUE_V1 ==="

cp src/scripts/build_market_universe_research_queue_v1.py \
   /tmp/build_market_universe_research_queue_v1.before_market_model_adoption.bak

python - <<'PY'
from pathlib import Path

p = Path("src/scripts/build_market_universe_research_queue_v1.py")
s = p.read_text()
...
cat > scripts/test_market_model_adoption_research_queue_v1.sh <<'SH_TEST'
#!/usr/bin/env bash
set -euo pipefail
...
chmod +x scripts/test_market_model_adoption_research_queue_v1.sh
scripts/test_market_model_adoption_research_queue_v1.sh

echo "VERDICT=BUILD_MARKET_MODEL_ADOPTION_RESEARCH_QUEUE_V1_OK"
