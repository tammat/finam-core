...
echo "=== TEST_EXPANDED_RUNTIME_CANDIDATES_V2 ==="

PYTHONPATH=src python -m py_compile \
  src/scripts/research/build_expanded_runtime_candidates_v2.py

OUT="$(PYTHONPATH=src python src/scripts/research/build_expanded_runtime_candidates_v2.py)"
...
echo "$OUT" | grep -q "VERDICT=EXPANDED_RUNTIME_CANDIDATES_V2_READY"
...
with psycopg2.connect(db) as conn:
    with conn.cursor() as cur:
        cur.execute("SELECT count(*) FROM analytics_global_edge_expanded_runtime_candidates_v2;")
...
print("db_expanded_runtime_candidates_ready=READY")
print("db_runtime_block_ready=READY")
PY

echo "VERDICT=TEST_EXPANDED_RUNTIME_CANDIDATES_V2_OK"
