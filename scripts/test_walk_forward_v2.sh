...
echo "=== TEST_WALK_FORWARD_V2 ==="

PYTHONPATH=src python -m py_compile \
  src/scripts/research/build_walk_forward_v2.py

OUT="$(PYTHONPATH=src python src/scripts/research/build_walk_forward_v2.py)"

echo "$OUT"

echo "$OUT" | grep -q "mode=research_only"
echo "$OUT" | grep -q "walk_forward_rows="
echo "$OUT" | grep -q "walk_window=WF_70_30_SYNTHETIC_V2"
...
echo "$OUT" | grep -q "VERDICT=WALK_FORWARD_V2_READY"
...
with psycopg2.connect(db) as conn:
    with conn.cursor() as cur:
        cur.execute("SELECT count(*) FROM analytics_global_edge_walk_forward_v2;")
...
print("db_walk_forward_ready=READY")
print("db_classification_ready=READY")
print("db_runtime_block_ready=READY")
PY

echo "VERDICT=TEST_WALK_FORWARD_V2_OK"
