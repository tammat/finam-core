...
echo "=== TEST_FUTURES_RS_BOTTOM_REVERSAL_FILTERS_FIX_V1 ==="

out="$(mktemp)"

python3 -m py_compile src/scripts/research/build_futures_rs_bottom_reversal_filters_fix_v1.py

PYTHONPATH=src \
RUNTIME_ALLOW_TRADING=0 \
EXECUTION_ENABLED=0 \
REAL_TRADING_ENABLED=0 \
python3 src/scripts/research/build_futures_rs_bottom_reversal_filters_fix_v1.py \
  --migrate --save | tee "$out"

grep -q "VERDICT=FUTURES_RS_BOTTOM_REVERSAL_FILTERS_FIX_READY" "$out"
grep -q "TEST_FUTURES_RS_BOTTOM_REVERSAL_FILTERS_FIX_V1_OK" "$out"
...
# Guard: после фикса BOTTOM1 ALL 240m должен быть положительным edge.
...
echo "VERDICT=FUTURES_RS_BOTTOM_REVERSAL_FILTERS_FIX_TEST_OK"
echo "TEST_FUTURES_RS_BOTTOM_REVERSAL_FILTERS_FIX_V1_OK"
