...
echo "=== BUILD_MARKET_MODEL_ADOPTION_FRESHNESS_V1 ==="

cp src/scripts/build_paper_edge_market_data_freshness_v1.py \
   /tmp/build_paper_edge_market_data_freshness_v1.before_market_model_adoption.bak
...
cat > scripts/test_market_model_adoption_freshness_v1.sh <<'SH_TEST'
#!/usr/bin/env bash
set -euo pipefail

echo "=== TEST_MARKET_MODEL_ADOPTION_FRESHNESS_V1 ==="
...
python src/scripts/build_paper_edge_market_data_freshness_v1.py \
  | tee /tmp/market_model_adoption_freshness_v1.txt

grep -q "market_source=marketcore.market_snapshot_v1 exists=1" \
  /tmp/market_model_adoption_freshness_v1.txt
...
fresh=$(psql -At -d finam_core -c "
SELECT count(*)
FROM marketcore_ui.paper_edge_market_data_freshness_v1
WHERE freshness_status IN ('CANDIDATE_BOUND_FRESH','SOURCE_FRESH');
")

bad_source=$(psql -At -d finam_core -c "
SELECT count(*)
FROM marketcore_ui.paper_edge_market_data_freshness_v1
WHERE source_table <> 'marketcore.market_snapshot_v1';
")
...
psql -d finam_core -c "
SELECT row_type, candidate_symbol, candidate_timeframe, market_symbol, market_timeframe,
       source_table, bars_total, latest_bar_ts, freshness_status
FROM marketcore_ui.paper_edge_market_data_freshness_v1
ORDER BY freshness_rank
LIMIT 30;
"

echo "freshness_rows=$rows"
...
echo "VERDICT=MARKET_MODEL_ADOPTION_FRESHNESS_V1_READY"
echo "VERDICT=TEST_MARKET_MODEL_ADOPTION_FRESHNESS_V1_OK"
SH_TEST

chmod +x scripts/test_market_model_adoption_freshness_v1.sh
scripts/test_market_model_adoption_freshness_v1.sh

echo "VERDICT=BUILD_MARKET_MODEL_ADOPTION_FRESHNESS_V1_OK"
