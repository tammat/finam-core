...
echo "=== BUILD_EDGE_OOS_USE_MARKET_UNIVERSE_V1 ==="

cp src/scripts/build_edge_oos_validation_v1.py /tmp/build_edge_oos_validation_v1.py.bak
cp src/scripts/build_edge_oos_backtest_v1.py /tmp/build_edge_oos_backtest_v1.py.bak

python - <<'PY'
from pathlib import Path

# OOS validation должен идти от новой robustness-таблицы,
# которая уже переведена на Market Universe.
p = Path("src/scripts/build_edge_oos_validation_v1.py")
s = p.read_text()
...
# Если старый код ожидал source_validation_rank — приводим к фактической схеме.
s = s.replace("source_validation_rank", "source_pipeline_rank")
s = s.replace("validation_status", "pipeline_status")

p.write_text(s)

# OOS backtest должен читать edge_oos_validation_v1,
# а не старые BR-only candidates/pipeline.
p = Path("src/scripts/build_edge_oos_backtest_v1.py")
s = p.read_text()
...
cat > scripts/test_edge_oos_use_market_universe_v1.sh <<'SH_TEST'
#!/usr/bin/env bash
set -euo pipefail
...
chmod +x scripts/test_edge_oos_use_market_universe_v1.sh
scripts/test_edge_oos_use_market_universe_v1.sh

echo "VERDICT=BUILD_EDGE_OOS_USE_MARKET_UNIVERSE_V1_OK"
