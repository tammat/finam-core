...
echo "=== TEST_EDGE_SCORE_MODEL_V2_SHADOW_OBSERVATION_DAILY_FINAL_AUDIT ==="
...
mkdir -p /tmp/finam_pycache_daily_final

for f in "${files[@]}"; do
    test -f "$f"
    PYTHONPYCACHEPREFIX=/tmp/finam_pycache_daily_final \
    PYTHONPATH=src \
    python -m py_compile "$f"
done

edge_score_rows=$(psql -At -d finam_core -c \
"select count(*) from analytics.edge_score_model_v2;")

reconciliation_rows=$(psql -At -d finam_core -c \
"select count(*) from analytics.edge_score_model_v2_reconciliation;")

explain_rows=$(psql -At -d finam_core -c \
"select count(*) from analytics.edge_score_model_v2_explain;")

shadow_rows=$(psql -At -d finam_core -c \
"select count(*) from analytics.edge_score_model_v2_shadow_observation_v1;")

daily_rows=$(psql -At -d finam_core -c \
"select count(*) from analytics.edge_score_model_v2_shadow_observation_daily_v1;")

test "$edge_score_rows" -gt 0
test "$reconciliation_rows" -gt 0
test "$explain_rows" -gt 0
test "$shadow_rows" -gt 0
test "$daily_rows" -gt 0
...
curl -fsS \
"http://127.0.0.1:8080/max-edge?v=$(date +%s)" \
>/tmp/max_edge_daily_final.html

curl -fsS \
"http://127.0.0.1:8080/edge-score-shadow?v=$(date +%s)" \
>/tmp/shadow_daily_final.html

curl -fsS \
"http://127.0.0.1:8080/edge-score-shadow-daily?v=$(date +%s)" \
>/tmp/shadow_daily_dashboard_final.html

grep -q edge-score-explain-card /tmp/max_edge_daily_final.html
grep -q edge-score-shadow-observation-card /tmp/shadow_daily_final.html
grep -q edge-score-shadow-daily-card /tmp/shadow_daily_dashboard_final.html
...
echo "VERDICT=EDGE_SCORE_MODEL_V2_SHADOW_OBSERVATION_DAILY_FINAL_AUDIT_READY"
echo "VERDICT=TEST_EDGE_SCORE_MODEL_V2_SHADOW_OBSERVATION_DAILY_FINAL_AUDIT_OK"
