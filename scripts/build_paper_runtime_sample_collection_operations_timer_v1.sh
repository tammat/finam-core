...
echo "=== BUILD_PAPER_RUNTIME_SAMPLE_COLLECTION_OPERATIONS_TIMER_V1 ==="

mkdir -p deploy/systemd src/scripts scripts

cat > src/scripts/run_paper_runtime_sample_collection_operations_cycle_v1.py <<'PY'
...
cat > deploy/systemd/finam-paper-sample-operations.service <<'UNIT'
[Unit]
Description=Finam Core Paper Runtime Sample Collection Operations Cycle V1
After=network.target postgresql.service finam-paper-sample-collection.service
...
ExecStart=/opt/finam-core/venv/bin/python src/scripts/run_paper_runtime_sample_collection_operations_cycle_v1.py

[Install]
WantedBy=multi-user.target
UNIT

cat > deploy/systemd/finam-paper-sample-operations.timer <<'UNIT'
[Unit]
Description=Run Finam Core Paper Runtime Sample Collection Operations Cycle V1 every 5 minutes
...
AccuracySec=30s
Persistent=true
Unit=finam-paper-sample-operations.service

[Install]
WantedBy=timers.target
UNIT

cat > scripts/test_paper_runtime_sample_collection_operations_timer_v1.sh <<'SH_TEST'
#!/usr/bin/env bash
set -euo pipefail
...
chmod +x scripts/test_paper_runtime_sample_collection_operations_timer_v1.sh

scripts/test_paper_runtime_sample_collection_operations_timer_v1.sh

echo "VERDICT=BUILD_PAPER_RUNTIME_SAMPLE_COLLECTION_OPERATIONS_TIMER_V1_OK"
