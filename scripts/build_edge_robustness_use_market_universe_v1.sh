...
echo "=== BUILD_EDGE_ROBUSTNESS_USE_MARKET_UNIVERSE_V1 ==="

mkdir -p src/scripts scripts

cp src/scripts/build_edge_robustness_check_v1.py /tmp/build_edge_robustness_check_v1.py.bak

python - <<'PY'
from pathlib import Path

p = Path("src/scripts/build_edge_robustness_check_v1.py")
s = p.read_text()
...
s = s.replace("pipeline_rank", "validation_rank")
s = s.replace("pipeline_status", "validation_status")
s = s.replace("pipeline_stage", "validation_stage")
...
cat > scripts/test_edge_robustness_use_market_universe_v1.sh <<'SH_TEST'
#!/usr/bin/env bash
set -euo pipefail
...
chmod +x scripts/test_edge_robustness_use_market_universe_v1.sh
scripts/test_edge_robustness_use_market_universe_v1.sh

echo "VERDICT=BUILD_EDGE_ROBUSTNESS_USE_MARKET_UNIVERSE_V1_OK"
