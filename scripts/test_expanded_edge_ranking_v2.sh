...
echo "=== TEST_EXPANDED_EDGE_RANKING_V2 ==="

PYTHONPATH=src python -m py_compile \
  src/scripts/research/build_expanded_edge_ranking_v2.py

OUT="$(PYTHONPATH=src python src/scripts/research/build_expanded_edge_ranking_v2.py)"

echo "$OUT"

echo "$OUT" | grep -q "mode=research_only"
echo "$OUT" | grep -q "expanded_ranking_rows="
...
echo "$OUT" | grep -q "VERDICT=EXPANDED_EDGE_RANKING_V2_READY"
...
with psycopg2.connect(db) as conn:
    with conn.cursor() as cur:
        cur.execute("SELECT count(*) FROM analytics_global_edge_expanded_ranking_v2;")
...
print("db_expanded_ranking_ready=READY")
print("db_rank_ready=READY")
print("db_runtime_block_ready=READY")
PY

echo "VERDICT=TEST_EXPANDED_EDGE_RANKING_V2_OK"
