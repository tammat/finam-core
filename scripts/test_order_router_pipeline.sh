...
routed = p._route_order_if_enabled(intent, market_state)

assert routed is not None, routed
assert routed["execution_action"] == "STOP", routed
assert routed["order_route"] == "STOP_ORDER", routed
...
blocked = p._route_order_if_enabled(intent, wide_spread)
assert blocked is None, blocked

print("ORDER_ROUTER_PIPELINE_OK")
PY
