...
echo "=== BUILD_PARAMETER_SEARCH_FOUNDATION_V1 ==="

mkdir -p sql/analytics src/scripts scripts

cat > sql/analytics/017_parameter_search_foundation_v1.sql <<'SQL'
...
cat > src/scripts/build_parameter_search_foundation_v1.py <<'PY'
...
cat > scripts/test_parameter_search_foundation_v1.sh <<'SH_TEST'
#!/usr/bin/env bash
set -euo pipefail
...
chmod +x scripts/test_parameter_search_foundation_v1.sh
scripts/test_parameter_search_foundation_v1.sh

echo "VERDICT=BUILD_PARAMETER_SEARCH_FOUNDATION_V1_OK"
