...
- Дедупликация реализована в источнике: устойчивый `signal_id` строится по
  инструменту, стратегии, стороне, таймфрейму и времени завершённой свечи.
  Цена текущей котировки и время повторного цикла в ключ не входят. Повторная
  оценка той же свечи прекращается до risk/shadow gates и больше не увеличивает
  `signals` или V5-статистику. Исторические 178 строк сохранены как аудит.
- Проверка дедупликации и связанных runtime-контрактов: 46 тестов успешно.
  Для загрузки исправления работающему процессу требуется штатный restart
  `finam-paper-pipeline.service`; REAL при этом остаётся выключен.
- На HOME добавлен блок «Сигналы сегодня»: семантически уникальные условия
  завершённых свечей, LONG/SHORT, время и русская причина Shadow/Paper-блокировки.
  Сводка отдельно показывает число уникальных условий, допущенных Paper и
  оставленных в Shadow; повторные циклы одной свечи на UI не выводятся.
