...
## Swing exit по закрытым свечам 28.07.2026

- Swing Paper-контур проверен отдельно от intraday: источником решений остаются только закрытые H1/H4/D1-бары из `analytics.swing_market_bars_v1`.
- Устранён разрыв между validation и Paper: `run_swing_paper_engine_v1.py` теперь использует общий `dynamic_exit_v1` вместо безусловного `HOLDING_PERIOD_EXIT`.
- Доступные причины закрытия: `ATR_STOP`, `ATR_TRAIL`, `TREND_GONE`, `VOLATILITY_RISK`, `MAX_HOLD`; фиксированный hold сохранён только для кандидатов с `FIXED_HOLD`.
- `exit_max_holding_bars` является аварийным горизонтом, а не главным выходом. Пока горизонт не достигнут и state-trigger отсутствует, открытая позиция остаётся открытой.
- LONG и SHORT используют симметричную логику; exit reason сохраняется без подмены в Paper order/trade.
- Проверки: `24 passed`; `git diff --check` — без ошибок.
- Службы, scheduler, миграции и торговое исполнение не запускались. Runtime-применение требует отдельного безопасного reload и наблюдения следующего закрытого swing-бара.
