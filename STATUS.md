...
Обновлено: 03.08.2026, 07:25 МСК.

## Открытие рынка 03.08.2026 — данные и индексы

- Finam начал отдавать утренние свечи около 07:05 МСК. Акции получают M5,
  приоритетные V5-инструменты — M1; процесс загрузки исправен.
- Устранён ложный `GAP` после выходных: контроль непрерывности сравнивает
  свечи только внутри текущей торговой сессии. На проверке 12 акций получили
  `READY` вместо ошибочного `GAP`.
- В контроль качества добавлены режимные источники `IMOEX2 M1`, `RVI M1` и
  `MXU6@RTSX M1`. Они диагностические и не требуют торговой спецификации затрат.
- `IMOEX2` в 07:22 МСК имеет статус `READY`; `MXU6` и `RVI` до открытия
  соответствующей сессии честно показываются как `OUT_OF_SESSION`.
- Загрузчик MOEX-индексов расширен RVI. История RVI загружена; отсутствие
  утреннего значения до основной сессии не подменяется фиктивной котировкой.
- MX добавлен в минутный fast-ingestion; контрольный запуск сохранил 22 свечи.
- Главный UI показывает яркий светофор на каждой строке: зелёный — норма,
  жёлтый — ожидание/накопление, красный — блокировка/ошибка. Добавлена строка
  «Состояние рынка» с вероятностями trend/range/shock, свежестью IMOEX2/MX/RVI
  и временем последнего подтверждённого контекста. Направление отображается
  как 🟢⬆️ рост, 🔴⬇️ снижение или 🟡➡️ флэт.
- Профильная проверка: 20 тестов успешно. REAL остаётся выключен; Paper-профили
  и замороженная V5-когорта не менялись.
- На 07:38 зарегистрировано 178 входных намерений и 0 исполнимых Paper-сигналов:
  140 закрыты отсутствием OOS-допуска, 38 — несвежим RVI до основной сессии.
  Частые повторные оценки одного M5-условия не считаются независимыми сигналами;
  их дедупликация остаётся ближайшей операционной задачей.
- Для постоянного включения MX в системный fast-loader требуется установить
  обновлённый unit из `deploy/systemd/finam-v5-bars-fast.service` и выполнить
  `systemctl daemon-reload`; код и unit зафиксированы в репозитории.

## Зафиксированный эксперимент V5 от 02.08.2026

- До исхода текущего эксперимента не менять четыре замороженные ветки:
  `SBER LONG`, `BRQ6 SHORT`, `GDU6/GLDRUBF LONG`, `CNYRUBF LONG`.
- Газ остаётся резервным Shadow и не подменяет зафиксированную V5-когорту.
- Не добавлять инструменты, стратегии и новые сетки параметров до результата
  этих веток. Изменения допускаются только для устранения дефекта данных,
  Research→Runtime parity, исполнения или risk-gate.
- Откат Paper допускается при нарушении parity, ошибке единиц/контрактной
  спецификации, отрицательном OOS-вердикте или срабатывании заранее заданного
  защитного критерия. Откат ради улучшения текущего P&L запрещён.
- Текущий статус: четыре V5 run в `COLLECTING`, все `0/20`; ExecutionSpec parity
  — четыре `NOT_PROVEN`; observation parity — 0. REAL выключен.
- Вероятностный режим `TREND/RANGE/SHOCK` и гистерезис работают только как
  Shadow-диагностика. Они не меняют Paper до отдельного parity/OOS PASS.

## Операционный статус 31.07.2026

- `finam-paper-pipeline`, `marketcore-ui-shell` и `marketcore-os` активны; HOME отвечает HTTP 200.
- REAL заблокирован: `EXECUTION_MODE=paper`, `EXECUTION_ENABLED=0`,
  `REAL_TRADING_ENABLED=0`, `REAL_EXECUTION_ENABLED=0`.
- Shadow обновляется внутри дня каждые 15 минут. В БД 7254 пары: 2287
  завершены, 762 ждут окончания независимого горизонта; последний результат
  18:47 МСК, последнее формирование 18:54 МСК.
- UI отдельно показывает свежесть всего Shadow-потока и дату последнего
  результата конкретного победителя; старая дата победителя больше не выдаётся
  за остановку всего расчёта.
- Из 6100 закрытых Paper-сделок 1035 имеют наблюдаемый `net_pnl_r`, рассчитанный
  после комиссий относительно первоначального стопа. История без сохранённого
  стопа остаётся честно ненаблюдаемой в R и не получает фиктивный ноль.
- Новые Paper-фьючерсные входы запрещены после 22:30 и до 07:00 МСК; выходы
  разрешены всегда. Это закрывает перенос виртуального стопа через ночной разрыв,
  который дал BRQ6 убыток -4,41R.
- Адаптивный Paper-пилот ограничен одной открытой позицией, объёмом не более 1
  и бюджетом до пяти закрытых сделок перед повторным решением.
- OOS run: 0, OOS PASS: 0, активных Paper-пилотов: 0, активных оптимизированных
  профилей: 0. Подтверждённого edge пока нет; REAL не разрешён.
...
## Checkpoint 29.07.2026, 22:50 МСК — UI, управляемая оптимизация и защита NG

- Runtime проверен на сервере `deb`: `finam-paper-pipeline`, `marketcore-ui-shell` и `finam-paper-safe` активны; UI слушает внутренний порт `8080` (внешняя публикация — `18080`).
- Safety подтверждена из systemd environment: `EXECUTION_MODE=paper`, `EXECUTION_ENABLED=0`, `REAL_TRADING_ENABLED=0`, `REAL_EXECUTION_ENABLED=0`.
- Холодный HOME-запрос ускорен: тяжёлый tick-level lookup плавающего P&L заменён индексированным чтением последних `market_bars`; измеренный свежий процесс — примерно `0,09–0,15 с`, кэшированный запрос — около `0,006 с`.
- В UI добавлен управляемый workflow оптимизатора: `Подтвердить Paper / Отклонить / Продолжить Shadow / Откатить Paper`; подтверждение допускается только после readiness/OOS-gates и не может включить REAL.
- Визуальная переработка workflow подготовлена локально, но намеренно ещё не развёрнута: вместо плоской сетки кнопок будут карточки с русским названием, направлением, прогрессом `из 80`, OOS `из 20`, параметрами и только релевантными действиями. Черновики сохранены в рабочем окружении Codex: `.codex-tmp/renderer_clarity.py` и `.codex-tmp/css_clarity.css`.
- Для NG новые входы переведены в fail-closed: они блокируются при отсутствии ровно одного разрешённого канонического runtime-контракта, при kill switch, неправильном контракте или cooldown; EXIT уже открытой позиции не блокируется.
- Runtime-проверка после рестарта зафиксировала `NG_RUNTIME_UNIVERSE_DISABLED enabled=NONE`; новых NG-сигналов после рестарта нет. Это безопасная блокировка, а не отсутствие рыночных данных.
- Сравнение ликвидности: `NGQ6` — 938 M1-баров и объём 457901; `NGU6` — 775 M1-баров и объём 51205. По данным и календарю текущим кандидатом является `NGQ6`, но governance сейчас явно запрещает его автоматическое включение.
- Последние зафиксированные коммиты: `bf50f4d8`, `c02852c2`, `8f3d393b`, `e230bbdb`; ветка `codex/research-edge-v5` синхронизирована с origin до обновления этого checkpoint.
- Следующий безопасный шаг: выяснить источник `governance_block/active_edge=false`, выбрать один канонический NG-контракт через штатную state machine, затем проверить входы только в Paper. После этого завершить и проверить карточный UI оптимизатора на desktop и телефоне.
