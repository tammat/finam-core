...
def test_research_dashboard_reads_persisted_system_runs_and_analysis() -> None:
    ...
    assert "edge_search_scenario_run_v1" in resolver
    assert "edge_search_step_run_v1" in resolver
    assert "edge_search_run_analysis_v1" in resolver
...
def test_research_dashboard_renders_compact_audit_table() -> None:
    ...
    assert 'columns=("status","started","steps","duration","outcome","reason","recommendation","analysis")' in renderer
