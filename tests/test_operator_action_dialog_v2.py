...
def test_research_dialog_closes_immediately_after_double_click_choice() -> None:
    source = Path("src/marketcore/presentation/ui_runtime/assets/v2/browser_platform_driver_v2.js").read_text()
    handler = source.split('button.addEventListener("click", async () => {', 1)[1]
    before_request = handler.split("await this.actionSink", 1)[0]
    assert "globalObject.confirm" not in before_request
