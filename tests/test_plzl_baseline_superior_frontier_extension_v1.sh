...
SCRIPT="src/scripts/research/build_plzl_baseline_superior_frontier_extension_v1.py"

echo "=== TEST PLZL BASELINE SUPERIOR FRONTIER EXTENSION V1 ==="
...
grep -q \
'^GRID_RESULT rank=1 .*code=PLZL_FRONTIER_S1.4_RM1.3 .*state=BASELINE_INFERIOR$' \
<<< "$OUTPUT"
...
grep -q \
'^VERDICT=PLZL_BASELINE_SUPERIOR_FRONTIER_EXTENSION_V1_READY$' \
<<< "$OUTPUT"

echo "VERDICT=TEST_PLZL_BASELINE_SUPERIOR_FRONTIER_EXTENSION_V1_OK"
