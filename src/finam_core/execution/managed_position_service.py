...
from finam_core.execution.position_registry import ManagedPosition, PositionRegistry
from finam_core.storage.managed_position_repository import ManagedPositionRepository


class ManagedPositionService:
    """
    Runtime + persistent state for managed positions.
    No broker calls here.
    """

    def __init__(self, registry=None, repository=None):
    ...
    def get(self, symbol: str) -> ManagedPosition | None:
        ...
        pos = self.repository.get(symbol)
        if pos is not None:
            self.registry.register_position(pos)
    ...
    def mark_breakeven_done(self, symbol: str, stop_order_id: str | None = None) -> ManagedPosition | None:
        pos = self.registry.mark_breakeven_done(symbol, stop_order_id=stop_order_id)
        if pos is not None:
            self.repository.save(pos)
        return pos

    def update_stop(self, symbol: str, stop_order_id: str | None) -> ManagedPosition | None:
        pos = self.registry.update_stop(symbol, stop_order_id)
        if pos is not None:
            self.repository.save(pos)
        return pos
