...
class SignalStage(WorkflowStage):
    # V1: безопасная заглушка SignalStage.
    # Реальная генерация сигналов будет подключена отдельным этапом.

    def execute(self, context: WorkflowContext) -> WorkflowResult:
        return WorkflowResult(
            status="OK",
            next_stage=context.next_stage,
            reason="SIGNAL_STAGE_STUB_OK",
            payload={
                "stage": "SIGNAL",
                "signal_logic": "STUB_ONLY",
