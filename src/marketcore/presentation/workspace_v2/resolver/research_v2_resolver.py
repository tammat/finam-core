    ...
    def resolve(self, *, generated_at=None):
        ...
        with psycopg2.connect("postgresql:///finam_core") as connection:
            with connection.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cursor:
                ...
                cursor.execute("""SELECT * FROM analytics.oos_remediation_branch_summary_v1
                    ORDER BY CASE branch_code WHEN 'COST_REMEDIATION' THEN 1 ELSE 2 END""")
                remediation_branches=tuple(OosRemediationBranchV1(
                    ...
                    int(row["oos_pass"] or 0),str(row["process_status"]),float(row["progress_pct"] or 0),
