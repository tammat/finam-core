...
class IntradayV2Resolver:
    def resolve(self, *, timezone_code="Europe/Moscow", generated_at=None):
        ...
        shadow_status=("WAITING_CANDIDATE" if not shadow["orders"] else
                       ...
                       "OBSERVING")
        return IntradaySnapshotV2(session_date,int(intraday["total"]),int(intraday["instruments"]),Decimal(intraday["pnl"]),_utc(intraday["last_at"]),str(paper.get("paper_status") or "UNAVAILABLE"),int(paper.get("signals_today") or 0),int(paper.get("fills_today") or 0),Decimal(paper.get("pnl_today") or 0),_utc(paper.get("refreshed_at")),shadow_status,int(shadow["orders"]),int(shadow["fills"]),int(shadow["positions"]),shadow_last,now)
