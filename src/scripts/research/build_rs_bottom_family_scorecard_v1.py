...
def main() -> int:
    ...
    print("=== RS_BOTTOM_FAMILY_SCORECARD_V1 ===")
            ...
            cur.execute("""
                ...
                features as (
                    select
                        b.*,
                        case
                            when symbol like 'BR%' then 'BRENT'
                            when symbol like 'NG%' then 'GAS'
                            when symbol like 'GD%' or symbol like 'GL%' then 'GOLD'
                            when symbol like 'SV%' then 'SILVER'
                            when symbol like 'USDRUB%' or symbol like 'CNYRUB%' then 'FX'
                            else 'OTHER'
                        end as family,
                ...
                from outcomes
                group by family, selection, filter_name, horizon_min
                having count(*) filter (where return_pct is not null) >= 50
                order by selection, filter_name, horizon_min, family
            """)
    ...
    if rows:
        print("VERDICT=RS_BOTTOM_FAMILY_SCORECARD_READY")
    else:
        print("VERDICT=RS_BOTTOM_FAMILY_SCORECARD_NO_DATA")

    print("TEST_RS_BOTTOM_FAMILY_SCORECARD_V1_OK")
    return 0
