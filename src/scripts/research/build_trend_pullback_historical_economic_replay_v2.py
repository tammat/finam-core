...
HASH_TIMEZONE = ZoneInfo("Europe/Moscow")
...
def load_bars(
    ...
    *,
    ...
    """
    Загружает строго frozen dataset V2 и fail-closed проверяет
    количество строк и SHA256 до построения canonical trades.
    """
    ...
    for row in rows:
        canonical_ts = row["ts"].astimezone(
            HASH_TIMEZONE
        )
    ...
    print(
        ...
        f"symbol={symbol} "
        f"bars={actual_count} "
        f"expected_bars={expected_count} "
        f"actual_hash={actual_hash} "
        f"expected_hash={expected_hash} "
        f"count_match={int(count_match)} "
        f"hash_match={int(hash_match)}"
    )

    if not count_match:
        raise RuntimeError(
            "ERROR=FROZEN_LINEAGE_COUNT_MISMATCH "
            f"symbol={symbol}"
        )

    if not hash_match:
        raise RuntimeError(
            "ERROR=FROZEN_LINEAGE_HASH_MISMATCH "
            f"symbol={symbol}"
        )
...
def main() -> int:
    ...
    lineage_cfg = json.loads(
        LINEAGE_CONFIG_PATH.read_text()
    )

    if lineage_cfg["timeframe"] != cfg["timeframe"]:
        raise RuntimeError(
            "ERROR=FROZEN_LINEAGE_TIMEFRAME_MISMATCH"
        )

    if not lineage_cfg.get("hash_contract_finalized"):
        raise RuntimeError(
            "ERROR=FROZEN_LINEAGE_HASH_CONTRACT_NOT_FINALIZED"
        )

    if not lineage_cfg.get("exact_replay_dataset_finalized"):
        raise RuntimeError(
            "ERROR=FROZEN_LINEAGE_DATASET_NOT_FINALIZED"
        )
    ...
    try:
        with conn.cursor(
            ...
            for symbol in SYMBOLS:
                lineage = lineage_cfg["symbols"][symbol]
                ...
                expected = (
                    lineage_cfg["expected_metrics_v2"][symbol]
                )

                trades_match = (
                    result.trades == int(expected["trades"])
                )

                net_pnl_match = (
                    abs(
                        result.net_pnl
                        - Decimal(expected["net_pnl"])
                    )
                    <= TOLERANCE
                )

                net_expectancy_match = (
                    abs(
                        result.net_expectancy
                        - Decimal(expected["net_expectancy"])
                    )
                    <= TOLERANCE
                )

                net_profit_factor_match = (
                    abs(
                        result.net_profit_factor
                        - Decimal(expected["net_profit_factor"])
                    )
                    <= TOLERANCE
                )
                ...
                print(
                    "HISTORICAL_ECONOMIC_REPLAY_V2_ROW "
                    f"symbol={symbol} "
                    ...
                    f"net_profit_factor={result.net_profit_factor} "
                    f"status={result.status} "
                    f"trades_match={int(trades_match)} "
                    f"net_pnl_match={int(net_pnl_match)} "
                    f"net_expectancy_match={int(net_expectancy_match)} "
                    f"net_profit_factor_match={int(net_profit_factor_match)} "
                    f"status_match={int(status_match)} "
                    f"match={int(case_match)}"
                )

                if not case_match:
                    raise RuntimeError(
                        "ERROR=HISTORICAL_REPLAY_V2_METRIC_MISMATCH "
                        f"symbol={symbol}"
                    )
