...
# Русский комментарий:
# TRADE_SOURCE_CLASSIFICATION_BACKFILL_PLAN_V1
# Dry-run план добавления payload.trade_source_class.
# Ничего не обновляет в БД.
# Цель — отделить clean runtime/paper от historical/legacy/dirty слоёв.
...
def main() -> int:
    ...
    print("=== TRADE SOURCE CLASSIFICATION BACKFILL PLAN V1 ===")
    print("mode=dry_run")
    ...
    print("TRADE_SOURCE_CLASSIFICATION_BACKFILL_PLAN_ROWS")
    ...
    for row in rows:
        ...
        if printed < SAMPLE_LIMIT:
            printed += 1
            print(
                "TRADE_SOURCE_CLASSIFICATION_BACKFILL_PLAN_ROW "
                f"id={row.get('id')} "
    ...
    print()
    print("TRADE_SOURCE_CLASSIFICATION_BACKFILL_CLASS_ROWS")
    for cls, count in class_counter.most_common():
        print(f"TRADE_SOURCE_CLASSIFICATION_BACKFILL_CLASS_ROW class={cls} rows={count}")

    print()
    print("TRADE_SOURCE_CLASSIFICATION_BACKFILL_REASON_ROWS")
    for reason, count in reason_counter.most_common():
        print(f"TRADE_SOURCE_CLASSIFICATION_BACKFILL_REASON_ROW reason={reason} rows={count}")

    print()
    print("TRADE_SOURCE_CLASSIFICATION_BACKFILL_PLAN_SUMMARY")
    ...
    if planned_updates > 0:
        print("VERDICT=TRADE_SOURCE_CLASSIFICATION_BACKFILL_PLAN_READY")
    else:
        print("VERDICT=TRADE_SOURCE_CLASSIFICATION_BACKFILL_NO_CHANGE_REQUIRED")

    print("TRADE_SOURCE_CLASSIFICATION_BACKFILL_PLAN_V1_OK")
    return 0
