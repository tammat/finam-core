...
def main() -> int:
    ...
    print("=== EQUITY_RS_BOTTOM_ABSOLUTE_V1 ===")
            ...
            cur.execute("""
                ...
                features as (
                    select
                        ...
                        b.close,
                        b.rn,
                        lag(b.close, 12) over (partition by b.symbol order by b.ts) as close_lookback
                    from bars b
                ),
                ranked as (
                    select
                ...
                selected as (
                    select 'BOTTOM1' as selection, * from ranked where bottom_rank = 1 and symbols_at_ts >= 3
                    union all
                    select 'BOTTOM3' as selection, * from ranked where bottom_rank <= 3 and symbols_at_ts >= 3
                    union all
                    select 'TOP1' as selection, * from ranked where top_rank = 1 and symbols_at_ts >= 3
                    union all
                    select 'TOP3' as selection, * from ranked where top_rank <= 3 and symbols_at_ts >= 3
                ),
                outcomes as (
                    select
                        s.selection,
    ...
    if not rows:
        print("VERDICT=EQUITY_RS_BOTTOM_ABSOLUTE_NO_DATA")
    else:
        print("VERDICT=EQUITY_RS_BOTTOM_ABSOLUTE_READY")

    print("TEST_EQUITY_RS_BOTTOM_ABSOLUTE_V1_OK")
    return 0
