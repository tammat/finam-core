...
def main() -> int:
    ...
    try:
        ...
        grid = [
            ("BRU6_LOCAL_S1.3_R1.4", 1.3, 1.4),
            ("BRU6_LOCAL_S1.4_R1.5", 1.4, 1.5),
            ("BRU6_LOCAL_S1.5_R1.6", 1.5, 1.6),
            ("BRU6_LOCAL_S1.6_R1.7", 1.6, 1.7),
            ("BRU6_LOCAL_S1.7_R1.8", 1.7, 1.8),
        ]
        ...
        for grid_index, (code, stop_atr, take_atr) in enumerate(grid):
            variant = Variant(
                ...
                stop_atr=stop_atr,
                take_atr=take_atr,
                policy_code="BRU6_CURRENT_CONTRACT_RESEARCH_V1",
                shadow_only=True,
            )
        ...
        print("=== BRU6 CURRENT CONTRACT NEIGHBORHOOD SEARCH V1 ===")
        print(f"source_bundles={len(bundles)}")
        print(f"completed_horizon_bundles={completed}")
        print(f"strategy={TARGET_STRATEGY}")
        print(f"symbol_group={TARGET_GROUP}")
        print(f"side={TARGET_SIDE}")
        print(f"grid_rows={len(grid)}")
        ...
        print(
            "VERDICT="
            "BRU6_CURRENT_CONTRACT_NEIGHBORHOOD_SEARCH_V1_READY"
        )
        return 0
