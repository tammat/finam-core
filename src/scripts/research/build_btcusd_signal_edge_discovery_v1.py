#!/usr/bin/env python3
"""
BTCUSD_SIGNAL_EDGE_DISCOVERY_V1

Цель:
найти или отвергнуть GROSS SIGNAL EDGE на BTCUSD.
...
следующий этап отдельно проверяет его исполнимость на подтверждённом торговом инструменте.
"""
...
CONFIG_PATH = Path(
    "config/research/btcusd_signal_edge_discovery_v1.json"
)
...
def main() -> int:
    ...
    print("=== BTCUSD SIGNAL EDGE DISCOVERY V1 ===")
    ...
    try:
        ...
        if total_bars < int(cfg["minimum_bars"]):
            print(
                "VERDICT="
                "BTCUSD_SIGNAL_EDGE_INSUFFICIENT_BARS"
            )
            return 3
