...
def main() -> int:
    ...
    try:
        ...
        grid = [
            (
                f"PLZL_LOCAL_S{stop:.1f}_RM{reward_mult:.1f}",
                stop,
                round(stop * reward_mult, 6),
            )
            for stop in (1.0, 1.1, 1.2, 1.3, 1.4)
            for reward_mult in (1.0, 1.1, 1.2)
        ]
        ...
        for grid_index, (code, stop_atr, take_atr) in enumerate(grid):
            variant = Variant(
                ...
                stop_atr=stop_atr,
                take_atr=take_atr,
                policy_code="PLZL_LOCAL_RESEARCH_V1",
                shadow_only=True,
            )
        ...
        print("=== PLZL NEIGHBORHOOD SEARCH V1 ===")
        ...
        print(
            "VERDICT="
            "PLZL_BASELINE_SUPERIOR_NEIGHBORHOOD_SEARCH_V1_READY"
        )
        return 0
