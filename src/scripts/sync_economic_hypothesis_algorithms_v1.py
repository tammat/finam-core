...
def dynamic(grid: list[dict]) -> list[dict]:
    output = []
    for item in grid:
        output.append({**item,"entry_policy_code":"META_ENTRY_V1","entry_trend_lookback":40,
                       "entry_volatility_lookback":20,"entry_volume_lookback":20,
                       "entry_min_trend_strength":0.15,"entry_max_volatility_z":2.5,
                       "entry_min_volume_ratio":0.75,"exit_policy_code":"DYNAMIC_EXIT_V1",
                       "exit_max_holding_bars":20,"exit_atr_lookback":14,
                       "exit_stop_atr":2.0,"exit_trailing_atr":2.5,
                       "exit_trend_lookback":20,"exit_volatility_lookback":20,
                       "exit_risk_volatility_z":3.0})
    return output
