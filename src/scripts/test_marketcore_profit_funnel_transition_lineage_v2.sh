...
test "$(psql -d finam_core -Atqc "SELECT count(*) FROM analytics.profit_funnel_transition_lineage_v2 WHERE transition_code='VALIDATED_EDGE_TO_OOS' AND lineage_status='BROKEN' AND reason_code='SOURCE_COHORT_ID_MISMATCH'")" -eq 1
echo "heuristic_links=0"
echo "forward_shadow_full_link=PASS"
echo "candidate_oos_gap_exposed=PASS"
