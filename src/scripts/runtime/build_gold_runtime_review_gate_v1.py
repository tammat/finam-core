    ...
    with psycopg2.connect(os.environ["DATABASE_URL"]) as conn:
        with conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cur:
            ...
            checks = {
                ...
                "stability_ratio_ok": float(row["stability_ratio"] if row["stability_ratio"] is not None else 0) >= 0.70,
                "negative_ratio_ok": float(row["negative_ratio"] if row["negative_ratio"] is not None else 1) <= 0.20,
                "effective_days_ok": int(row["days_effective"] or 0) >= 3,
                "filtered_pnl_positive": float(row["filtered_pnl"] if row["filtered_pnl"] is not None else 0) > 0,
