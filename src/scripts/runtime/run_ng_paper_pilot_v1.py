...
def main() -> int:
    ...
    parser.add_argument("--symbol", default=os.getenv("NG_PAPER_PILOT_SYMBOL", "NGM6@RTSX"))
    ...
    with psycopg.connect(build_psycopg_url(), row_factory=dict_row) as conn:
        with conn.cursor() as cur:
            ...
            profiles = {
                (str(row["side"]), int(row["hour_msk"])): dict(row)
    ...
    print("NG_PAPER_PILOT_V1_OK", flush=True)
    return 0
