...
DB=os.getenv("DATABASE_URL","postgresql:///finam_core"); SOURCE_VERSION="MOEX_NATURAL_GAS_ONLINE_V1"; MOSCOW=ZoneInfo("Europe/Moscow")

def fetch(secid:str,date_from:str,date_till:str)->list[dict[str,Any]]:
    rows=[]; start=0
    while True:
        r=requests.get(f"https://iss.moex.com/iss/engines/futures/markets/forts/securities/{secid}/candles.json",
            params={"from":date_from,"till":date_till,"interval":1,"start":start,"iss.meta":"off"},timeout=20)
        r.raise_for_status(); block=r.json().get("candles",{}); raw=block.get("data",[])
        rows.extend(dict(zip(block.get("columns",[]),item)) for item in raw)
        if len(raw)<100: break
        start+=len(raw); time.sleep(.1)
    return rows

def number(value:Any): return Decimal(str(value)) if value is not None else None
...
def main()->None:
    ...
    with psycopg2.connect(DB) as conn:
        with conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cur:
            ...
            cur.execute("""SELECT contract_symbol,expiration_date FROM public.futures_contract_universe
                WHERE root_symbol='NG' AND is_active AND status<>'QUARANTINE' AND expiration_date>=current_date
                ORDER BY expiration_date LIMIT 3""")
            for contract in cur.fetchall():
    ...
    print(json.dumps({"status":"ONLINE","rows":output},ensure_ascii=False)); print("VERDICT=MOEX_NATURAL_GAS_ONLINE_V1_OK")
