...
# Русский коммент: прогон исторических баров через PAPER pipeline
...
from finam_core.portfolio.portfolio_manager import PortfolioManager
from finam_core.portfolio.position_manager import PositionManager
from finam_core.risk.risk_engine import RiskEngine
from finam_core.storage.postgres_loader import PostgresLoader


def run_replay(symbol: str):
    print(f"REPLAY START symbol={symbol}")

    bus = EventBus()
    pm = PositionManager()
    portfolio = PortfolioManager(position_manager=pm)
    risk = RiskEngine()
    ...
    pipeline = PaperTradingPipeline(
        ...
        risk=risk,
        paper=paper,
        strategy=None  # не нужен, BR внутри pipeline
    )
