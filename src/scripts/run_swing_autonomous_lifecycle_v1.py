...
def forward_result(q,item,result):
  ...
  passed=n>=10 and pf>=1.05 and expectancy>0
  return passed,{"bars":len(timestamps),"trades":n,"profit_factor":pf,"expectancy":expectancy,"start":timestamps[0].isoformat(),"end":timestamps[-1].isoformat()}
  ...
  with psycopg2.connect(DB) as conn:
    with conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as q:
      ...
      for process in q.fetchall():
        ...
        passed=not int(e["unsafe"]) and e["profit_factor"]>=float(POLICY["minimum_net_profit_factor"]) and float(e["expectancy"])>float(POLICY["minimum_net_expectancy"])
        if not passed:
          q.execute("UPDATE analytics.swing_candidate_lifecycle_v1 SET status_code='FAIL',progress_pct=100,gate_evidence=gate_evidence||%s,reason_codes='[\"SHADOW_FAIL\"]',updated_at=clock_timestamp() WHERE process_id=%s",(psycopg2.extras.Json(safe({"shadow":e})),process["process_id"])); continue
        ...
        q.execute("UPDATE analytics.swing_shadow_cohort_v1 SET paper_allowed=true WHERE swing_shadow_cohort_id=%s",(process["shadow_cohort_id"],))
