    ...
    with c.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as q:
      ...
      for i in items:
        ...
        q.execute("""INSERT INTO analytics.swing_final_oos_result_v1(result_id,plan_item_id,holdout_fingerprint,holdout_start,holdout_end,trades,profit_factor,expectancy,folds_passed,adjusted_p_value,stressed_expectancy,capacity_rub,portfolio_correlation,statistical_pass,robustness_pass,holdout_pass,execution_pass,capacity_pass,portfolio_pass,verdict_code,promotion_allowed,reason_codes,execution_policy)
          ...
          (str(rid),str(i["plan_item_id"]),fp,ts[0],ts[-1],n,pf,exp,folds,ap,stressed,cap,portfolio_corr,gates["statistical"],gates["robustness"],gates["holdout"],gates["execution"],gates["capacity"],gates["portfolio"],"PASS" if ok else "FAIL",ok,psycopg2.extras.Json(reasons),psycopg2.extras.Json(json_safe({**execution,"portfolio_overlap_days":overlap}))))
        q.execute("UPDATE analytics.swing_next_research_plan_item_v1 SET status_code=%s,evaluated_at=clock_timestamp(),updated_at=clock_timestamp() WHERE plan_item_id=%s",("EVALUATED_PASS" if ok else "EVALUATED_FAIL",str(i["plan_item_id"])))
