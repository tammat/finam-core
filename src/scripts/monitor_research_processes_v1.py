...
def main():
        ...
        with connection.cursor() as cursor:
            ...
            cursor.execute("""WITH stale AS (
              ...
              WHERE (status='PENDING' AND requested_at<clock_timestamp()-interval '60 minutes')
                 OR (status='RUNNING' AND started_at<clock_timestamp()-interval '30 minutes')
            ...
            cursor.execute("""WITH stale AS (
              ...
              WHERE p.status_code='RUNNING' AND p.updated_at<clock_timestamp()-interval '30 minutes'
    ...
    print(f"command_events={command_events} process_events={process_events}")
